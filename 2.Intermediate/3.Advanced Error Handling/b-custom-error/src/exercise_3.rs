...
impl Display for CalculationError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let msg = match self {
            Self::Overflow => String::from("Overflow while performing the operation"),
            Self::DivideByZero { dividend } => format!(
                "Can not divide by zero. Attempting to divide {} by 0",
                dividend
            ),
            Self::InvalidOperand(operand) => {
                format!("{} is not a valid integer in range [-128, 127]", operand)
            }
            Self::InvalidOperation(msg) => {
                format!("{} is not a valid operation. Allowed: +,-,/,*", msg)
            }
        };
